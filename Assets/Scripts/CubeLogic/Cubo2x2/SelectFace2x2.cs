...

{
    ...
    public void AttemptPickup(GameObject face, Vector3 dragVector)
    {
        ...
        Transform sideCenter = currentSide[0].transform.parent;
        ...
        foreach (var side in allSides)
        {
            if (side.Contains(face))
            {
                ...
                Transform center = side[0].transform.parent;

                Vector3 sideAxis = (side[0].transform.parent.transform.position - readCube.transform.position).normalized;
        ...
        if (sliceToMove != null)
        {
            ...
            sliceToMove[0].transform.parent.GetComponent<PivotRotation2x2>().Rotate(sliceToMove);
        }
    }
}
